...
function addBulkRow() {
    ...
    tr.innerHTML = `
        ...
        <td class="p-2"><input type="text" class="form-input text-sm p-1.5 bulk-name" placeholder="Name" required></td>
        ...
        <td class="p-2">
            <label class="flex flex-col items-center justify-center w-20 h-16 border-2 border-dashed border-gray-300 rounded cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors group relative overflow-hidden" title="Upload Image">
                <img class="bulk-img-preview absolute inset-0 w-full h-full object-cover rounded hidden">
                <div class="bulk-img-placeholder flex flex-col items-center">
...
document.getElementById('bulk-add-form').addEventListener('submit', async function(e) {
    ...
    if (!items.length) { alert('No valid products to save.'); return; }

    const formData = new FormData();
    formData.append('csrf_token', document.getElementById('csrf').value);
    formData.append('company_id', document.getElementById('bulk-company').value);
    formData.append('items', JSON.stringify(items));

    // Attach images keyed by row index and send indices
    rows.filter(tr => tr.querySelector('.bulk-name').value).forEach(tr => {
        const idx = tr.querySelector('input[name="row_indices[]"]').value;
        formData.append('row_indices[]', idx);
        const fileInput = tr.querySelector('.bulk-img-input');
        if (fileInput && fileInput.files[0]) {
            formData.append('images[' + idx + ']', fileInput.files[0]);
        }
    });
    ...
    try {
        ...
        try { 
            ...
            btn.disabled = false; btn.innerHTML = 'Save All Products'; 
            return; 
        }
        if (data.success) location.reload();
        else { alert('Error: ' + data.message); btn.disabled = false; btn.innerHTML = 'Save All Products'; }
    } catch(err) { alert('Network error: ' + err.message); btn.disabled = false; btn.innerHTML = 'Save All Products'; }
});
