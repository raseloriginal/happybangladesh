...
/* ── SR cards sidebar ────────────────────────────────────────── */
#liveSrList::-webkit-scrollbar, .scrollbar-styled::-webkit-scrollbar {
    width: 6px; height: 6px;
}
#liveSrList::-webkit-scrollbar-track, .scrollbar-styled::-webkit-scrollbar-track {
    background: transparent;
}
#liveSrList::-webkit-scrollbar-thumb, .scrollbar-styled::-webkit-scrollbar-thumb {
    background: #cbd5e1; border-radius: 4px;
}
#liveSrList::-webkit-scrollbar-thumb:hover, .scrollbar-styled::-webkit-scrollbar-thumb:hover {
    background: #94a3b8;
}
...
.sr-card:hover, .sr-card.selected { border-color: #2563eb; box-shadow: 0 0 0 3px rgba(37,99,235,.08); }
.sr-card.selected { background: #f8fafc; }
.sr-card .avatar {
...
<div id="tab-live" class="track-tab-panel active">
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div class="space-y-2 lg:max-h-[600px] lg:overflow-y-auto pr-1" id="liveSrList">
      ...
      <div id="liveSrCards" class="space-y-2"></div>
    </div>
...
// ── Config ────────────────────────────────────────────────────
const BASE = '<?= BASE_URL ?>';
const API_LIVE    = BASE + '/dealer/api/sr-tracking/live';
const API_HISTORY = BASE + '/dealer/api/sr-tracking/history';
...
const storedProvider = localStorage.getItem('dealer_sr_tracking_map_provider') || 'osm';
setMapProvider('all', storedProvider);

function setMapProvider(mapType, providerKey) {
  localStorage.setItem('dealer_sr_tracking_map_provider', providerKey);
...
function updateCard(card, sr) {
  const onl = sr.is_online;
  const badge = card.querySelector('.badge');
  badge.className = `badge ${onl ? 'badge-online' : 'badge-offline'}`;
  badge.textContent = onl ? 'Online' : 'Offline';
  
  const meta = card.querySelector('.sr-meta');
  const latLngText = sr.lat ? `${sr.lat.toFixed(5)}, ${sr.lng.toFixed(5)}` : 'No location yet';
  const addressText = sr.address ? (sr.address.length > 40 ? escHtml(sr.address.slice(0, 40)) + '…' : escHtml(sr.address)) : '—';
  const agoText = sr.recorded_at ? timeAgo(sr.recorded_at) : 'Never';
  
  meta.innerHTML = `
    ${latLngText}<br>
    ${addressText}<br>
    <span class="text-gray-400 font-semibold time-ago-text" data-time="${sr.recorded_at || ''}">${agoText}</span>
  `;
}
...
function renderLive(srs) {
  ...
  document.getElementById('liveOnlineCount').textContent = onlineCount + ' Online / ' + srs.length + ' Total';

  const container = document.getElementById('liveSrCards');
  const currentIds = new Set(srs.map(s => s.id));
  
  // DOM Diffing - Clean up cards that no longer exist
  const existingCards = container.querySelectorAll('.sr-card');
  ...
  srs.forEach(sr => {
    ...
    let card = container.querySelector(`.sr-card[data-id="${sr.id}"]`);
    if (!card) {
      card = document.createElement('div');
      card.className = 'sr-card';
      card.dataset.id = sr.id;
      card.innerHTML = `
      ...
      `;
      card.addEventListener('click', () => selectSr(sr));
      container.appendChild(card);
    }
    
    // Toggle active state without layout jump
    card.classList.toggle('selected', selectedSrId === sr.id);
    updateCard(card, sr);

    // 2. Map Marker (Smooth transition on coordinate updates)
    if (sr.lat && sr.lng) {
      if (liveMarkers[sr.id]) {
        liveMarkers[sr.id].setLatLng([sr.lat, sr.lng]).setIcon(makeSrIcon(onl, sr.name));
      } else {
        liveMarkers[sr.id] = L.marker([sr.lat, sr.lng], { icon: makeSrIcon(onl, sr.name) })
          .addTo(liveMap)
