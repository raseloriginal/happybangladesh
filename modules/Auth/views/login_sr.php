...
<div class="min-h-screen bg-slate-100 flex items-center justify-center p-4">
  <div class="max-w-md w-full bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden p-8">
    ...
    <div class="text-center mb-8">
      <div class="w-14 h-14 bg-blue-800 rounded-2xl flex items-center justify-center text-white text-xl font-bold mx-auto mb-3 shadow-md shadow-blue-700/20">
        <i class="fa-solid fa-briefcase"></i>
      </div>
      <h1 class="text-2xl font-bold text-slate-900">Happy Bangladesh</h1>
      <p class="text-xs text-slate-500 mt-1 font-medium">Sales Rep Portal Sign In</p>
    </div>
    ...
    <!-- Login Form -->
    <form action="<?= url('sr/login') ?>" method="POST" class="space-y-4">
      <?= Helpers::csrfField() ?>
      
      <div>
        <label class="block text-xs font-bold text-slate-700 mb-1.5">Phone or Email</label>
        <input type="text" name="email" required placeholder="01700000000 / sr@happybd.com" 
      ...
      <div class="flex items-center gap-2 mt-1">
        <input type="checkbox" name="remember" value="1" id="remember_sr"
               class="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500 cursor-pointer">
        <label for="remember_sr" class="text-xs text-slate-600 font-medium cursor-pointer select-none">Remember me for 30 days</label>
      </div>
