<style>
  ...
  .sr-prod-grid-card:hover {
    border-color: #93c5fd !important;
  }
  .sr-prod-grid-card:active {
    transform: scale(0.98) !important;
  }
  .sr-prod-grid-img-wrap {
  ...
  .sr-prod-grid-card:hover .sr-prod-grid-img {
  ...
  /* Custom Confirm Modal Styling - modern high contrast full top overlay */
  .sr-modal-overlay {
...
<div class="sr-modal-overlay" id="confirmModalOverlay">
  <div class="sr-confirm-modal" onclick="event.stopPropagation()">
    ...
    <div class="sr-confirm-actions">
      ...
      <button class="sr-confirm-btn-yes" id="confirmModalYesBtn" type="button">হ্যাঁ, সংশোধন</button>
    </div>
  </div>
</div>
...
══════════════════════════════════════════════════════════════ -->
<div class="sr-sheet-overlay" id="srFreeItemOverlay" style="z-index: 1700;" onclick="closeSheet('srFreeItemSheet','srFreeItemOverlay')"></div>
<div class="sr-bottom-sheet-v2" id="srFreeItemSheet" style="z-index: 1701; max-height: 100dvh; display: flex; flex-direction: column;">
  <div class="sr-sheet-handle-v2"></div>
...
let shopViewMode = localStorage.getItem('sr_shop_view_mode') || 'grid';
...
function renderProductsGrid() {
  ...
  // ── 2-COLUMN GRID VIEW ──────────────────────────────────────
  if (shopViewMode === 'grid') {
    let gridHtml = `<div class="sr-prod-grid-layout">`;
    filteredProducts.forEach((p) => {
      ...
      const stockHtml = stockQty > 0 
        ? `<span class="inline-block text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">${stockQty} টি</span>`
        : `<span class="inline-block text-[10px] font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded border border-rose-200">স্টক নেই</span>`;

      const gridImgHtml = (p && p.image)
        ? `<img src="${BASE_URL}/${escHtml(p.image)}" class="sr-prod-grid-img" alt="${escHtml(p.name)}" loading="lazy" onerror="this.outerHTML='<div class=\\'w-full h-full flex items-center justify-center text-slate-300\\'><i class=\\'fa-regular fa-image text-lg\\'></i></div>'">`
      ...
      const btnHtml = isInCart 
        ? `<button type="button" class="w-full py-1.5 px-2 text-[11px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-300 rounded-lg hover:bg-emerald-100 active:scale-95 transition flex items-center justify-center gap-1" onclick="event.stopPropagation(); openProductSheet(${origIdx})"><i class="fa-solid fa-circle-check text-[10px]"></i> যোগ হয়েছে</button>`
        : `<button type="button" class="w-full py-1.5 px-2 text-[11px] font-bold text-blue-600 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 active:scale-95 transition flex items-center justify-center gap-1" onclick="event.stopPropagation(); openProductSheet(${origIdx})"><i class="fa-solid fa-plus text-[10px]"></i> যোগ করুন</button>`;

      gridHtml += `
        <div class="sr-prod-grid-card" onclick="openProductSheet(${origIdx})">
          <div class="flex items-center justify-between gap-1">
            ${stockHtml}
            ${isInCart ? '<span class="w-2.5 h-2.5 rounded-full bg-emerald-500 shrink-0" title="কার্টে যোগ করা হয়েছে"></span>' : ''}
          </div>
          
          <div class="sr-prod-grid-img-wrap">
            ${gridImgHtml}
          </div>
          
          <div class="mt-auto space-y-1.5">
            <div class="text-xs font-bold text-slate-800 sr-line-clamp-2 leading-tight" style="min-height:28px;" title="${escHtml(p.name)}">
              ${escHtml(p.name)}
            </div>
  ...
  // ── TABLE LIST VIEW ──────────────────────────────────────────
  let tableHtml = `
    <div style="overflow:hidden; border-radius:12px; border:1px solid #e2e8f0; background:#fff;">
      <table style="width:100%; text-align:left; border-collapse:collapse; table-layout:fixed; font-size:12px;">
        <colgroup>
          <col style="width:60%">
          <col style="width:20%">
          <col style="width:20%">
        ...
        <thead>
          <tr style="background:#f8fafc; border-bottom:1px solid #e2e8f0; color:#94a3b8; font-weight:700; font-size:10px; text-transform:uppercase; letter-spacing:0.05em; user-select:none;">
            <th style="padding:10px 12px; border-right:1px solid #e2e8f0;">প্রোডাক্ট নাম</th>
            <th style="padding:10px 8px; text-align:center; border-right:1px solid #e2e8f0;">স্টক</th>
            <th style="padding:10px 8px; text-align:center;">অ্যাকশন</th>
          </tr>
        </thead>
        <tbody>
  `;
  ...
  filteredProducts.forEach((p) => {
    ...
    const isInCart = cart.some(item => item.id === p.id);
    const btnHtml = isInCart 
      ? `<button style="padding:4px 8px; font-size:10px; font-weight:700; color:#059669; background:#ecfdf5; border:1px solid #a7f3d0; border-radius:6px; cursor:pointer; white-space:nowrap;" onclick="event.stopPropagation(); openProductSheet(${origIdx})">✓ যোগ হয়েছে</button>`
      : `<button style="padding:4px 8px; font-size:10px; font-weight:700; color:#2563eb; background:#eff6ff; border:1px solid #bfdbfe; border-radius:6px; cursor:pointer; white-space:nowrap;" onclick="event.stopPropagation(); openProductSheet(${origIdx})">+ যোগ করুন</button>`;
      
    const stockQty = parseInt(p.stock || 0);
    const stockHtml = stockQty > 0 
      ? `<span style="display:inline-block; font-size:9px; font-weight:800; color:#059669; background:#ecfdf5; padding:2px 6px; border-radius:4px; border:1px solid #a7f3d0;">${stockQty} টি</span>`
      : `<span style="display:inline-block; font-size:9px; font-weight:800; color:#e11d48; background:#fff1f2; padding:2px 6px; border-radius:4px; border:1px solid #fecdd3;">স্টক নেই</span>`;

    const imgHtml = (p && p.image)
      ? `<div style="width:40px; height:40px; border-radius:8px; background:#f8fafc; border:1px solid #e2e8f0; flex-shrink:0; display:flex; align-items:center; justify-content:center; overflow:hidden; padding:2px;"><img src="${BASE_URL}/${escHtml(p.image)}" style="max-width:100%; max-height:100%; object-fit:contain;" alt="" loading="lazy" onerror="handleProductImageError(this)"></div>`
      : `<div style="width:40px; height:40px; border-radius:8px; background:#f1f5f9; display:flex; align-items:center; justify-content:center; color:#cbd5e1; flex-shrink:0; border:1px solid #e2e8f0;"><i class="fa-regular fa-image" style="font-size:11px;"></i></div>`;

    tableHtml += `
        <tr style="border-bottom:1px solid #f1f5f9; cursor:pointer; transition:background 0.1s;" onclick="openProductSheet(${origIdx})" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background=''">
          <td style="padding:10px 12px; border-right:1px solid #f1f5f9; overflow:hidden;">
            <div style="display:flex; align-items:center; gap:10px; min-width:0;">
              ${imgHtml}
              <span style="font-weight:600; color:#1e293b; font-size:12px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; min-width:0; flex:1;">${escHtml(p.name)}</span>
            </div>
          </td>
